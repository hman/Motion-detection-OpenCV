...
sounds = [
    "Turret_turret_autosearch_1.wav",
    "Turret_turret_autosearch_2.wav",
    "Turret_turret_autosearch_4.wav",
    "Turret_turret_autosearch_5.wav",
    "Turret_turret_autosearch_6.wav"
]
...
class MotionDetectorInstantaneous():
    ...
    def __init__(self,threshold=8, doRecord=True, showWindows=True):
        ...
        #self.cam.set(cv.CV_CAP_PROP_FRAME_HEIGHT, 488)


        #self.capture=cv.CaptureFromCAM(-1)
        #self.frame = cv.QueryFrame(self.capture) #Take a frame to init recorder
        #self.frame = cv.fromarray(self.cam.read()[1])
        self.frame = cv.fromarray(cam.getDepth().getNumpyCv2())
        if doRecord:
            self.initRecorder()
    ...
    def run(self):
        ...
        while True:
            ...
            #curframe = cv.fromarray(self.cam.read()[1])
            curframe = cv.fromarray(cam.getDepth().getNumpyCv2())
            ...
            if self.somethingHasMoved():
                ...
                if not self.playing:
                    self.playing = True
                    nr = random.randrange(len(sounds) - 1)
                    subprocess.call(["aplay", sounds[nr]])
                    self.playing = False
                #cv.SaveImage('motion-%s.jpg' %  datetime.now().isoformat(), curframe)
            else:
                print 'nothing...'
...
if __name__=="__main__":
    detect = MotionDetectorInstantaneous(doRecord=False, showWindows=False)
    detect.run()
